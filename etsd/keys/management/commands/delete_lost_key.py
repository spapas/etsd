...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "fingerprint",
            type=str,
            help="The fingerprint of the lost/compromised private key",
        )
        # Optional arg
        parser.add_argument(
            "-d",
    ...
    def handle(self, *args, **kwargs):
        ...
        if not del_key:
            self.stdout.write(
                self.style.ERROR("No key with fingerprint: %s in database" % fingerp)
            )
            return
        ...
        if kwargs["delete"]:
            ...
            self.stdout.write(
                "Deleting key with fingerprint: %s of user: %s"
                % (del_key.fingerprint, del_key.user_id)
            )
            ...
            if del_cdata:
                ...
                for cdata in del_cdata:
                    self.stdout.write(
                        "Cipher data: %s , of message: %s (id %s)"
                        % (cdata.cipher_data, cdata.data.message, cdata.id)
                    )
                    cdata.delete()
            else:
            ...
            self.stdout.write(
                "Running this command with '-d' option will delete the following:"
            )
            self.stdout.write(
                "Key with fingerprint: %s of user: %s"
                % (del_key.fingerprint, del_key.user_id)
            )
            if del_cdata:
