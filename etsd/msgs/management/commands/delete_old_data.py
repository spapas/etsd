...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if del_cdata:
            ...
            self.stdout.write("{}".format(", ".join(str(x) for x in messages_to_delete)))
        ...
        if messages_to_notify:
            ...
            self.stdout.write("{}".format(", ".join(str(x) for x in messages_to_notify)))
        ...
        for k in participants_to_notify_dict:
            email_body = send_mail_body(
                ...
                dict(messages=participants_to_notify_dict[k], one_year_before_p10=one_year_before_p10),
            )
            ...
            if kwargs['no_dry'] and len(recipients) > 0:
                ...
                self.stdout.write("Will send emails to: {} for authority {}".format(",".join(recipients) or '-', k))
